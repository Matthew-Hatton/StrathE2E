...
weight_trophic_biomass <- function(model,network_indices,hr_length){
  weighted_trophic <- vector(mode = "list",hr_length^2)  #initialise weighted trophic storage
  
  for (i in seq(1,hr_length,1)){
    biomass_strip <- model[[i]]$final.year.outputs$mass_results_wholedomain[,1]
    trophic_levels <- network_indices[[i]][1:33,] #initialise trophic levels array & biomass array, pair each value
    ...
    weighted_trophic[[i]] = weighted_trophic_sums
    #cleanup
